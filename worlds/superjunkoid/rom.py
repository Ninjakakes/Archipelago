...
from worlds.superjunkoid.patch_utils import get_gen_data, ips_patch_from_file, get_multi_patch_path, patch_item_sprites, \
...
class SuperJunkoidDeltaPatch(APDeltaPatch):
    hash = SMJUHASH
    game = "Super Junkoid"
    patch_file_ending = ".apsj"
...
def write_rom_from_gen_data(gen_data_str: str, output_rom_file_name: str) -> None:
    ...
    rom_writer = RomWriter.fromFilePath(base_rom_path)  # this patches SM to Super Junkoid 1.3
    ...
    for loc in gen_data.sj_game.all_locations.values():
        if loc["hiddenness"] == "open":
            plmid = AP_ITEM[1]
        elif loc["hiddenness"] == "eye":
            plmid = AP_ITEM[2]
        else:
            plmid = AP_ITEM[3]
